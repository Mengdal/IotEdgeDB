...
const debugPprofEnvVar = "IEDB_DEBUG_PPROF"
...
// for the rationale.
const debugPprofAddrEnvVar = "IEDB_DEBUG_PPROF_ADDR"
...
const debugPprofAllowNonLoopbackEnvVar = "IEDB_DEBUG_PPROF_ALLOW_NON_LOOPBACK"
...
// listener when IEDB_DEBUG_PPROF is truthy. The listener is bound to
...
// NOT undo the default-mux registration. IEDB safely avoids the side
...
func startDebugPprofIfEnabled(coord *shutdown.Coordinator, logger zerolog.Logger) {
	// The caller (cmd/iedb/main.go) passes logger.Get("debug-pprof"), which
	...
	// matches the fail-closed precedent set by sandbox.
	...
	if !isLoopbackBindAddr(addr) {
		...
		// in via IEDB_DEBUG_PPROF_ALLOW_NON_LOOPBACK so this isn't a
...
// interface. Used to decide whether IEDB_DEBUG_PPROF_ALLOW_NON_LOOPBACK is
