/*
 * iedb Sustained Load Benchmark - C Client
 *
 * A high-performance HTTP benchmark client using libcurl and msgpack.
 * Uses the same MessagePack columnar format as iedb's /api/v1/write/msgpack endpoint.
 *
 * Build:
 ...
 *
 * Format matches iedb's expected structure:
 * {
 *   "m": "measurement_name",
 *   "columns": {
 *     "time": [int64, int64, ...],
...
int main(int argc, char *argv[]) {
    ...
    if (cfg.token) {
        ...
        printf("No IEDB_TOKEN set - authentication may fail\n");
    }
    ...
    headers = curl_slist_append(headers, "x-iedb-database: production");
