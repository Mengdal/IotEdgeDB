// Log ingestion benchmark for iedb vs Elasticsearch, OpenSearch, ClickHouse, VictoriaLogs, Loki, Quickwit
...
type Config struct {
	...
	Port        int
	Index       string
	Protocol    string // For iedb: msgpack or lineprotocol
	Token       string
...
func formatForIEDB(records []LogRecord, compress string, zstdEncoder *zstd.Encoder) []byte {
	n := len(records)
...
func generateBatches(cfg *Config) [][]byte {
	...
	for i := 0; i < cfg.Pregenerate; i++ {
		...
		switch cfg.Target {
		case TargetIEDB:
			batches[i] = formatForIEDB(records, cfg.Compress, zstdEncoder)
...
func worker(id int, cfg *Config, batches [][]byte, stats *Stats, client *http.Client) {
	...
	switch cfg.Target {
	case TargetIEDB:
		headers["x-iedb-database"] = cfg.Index
		if cfg.Token != "" {
...
func main() {
	...
	flag.StringVar(&cfg.Target, "target", "iedb", "Target: iedb, elastic, opensearch, clickhouse, clickhouse-native, victorialogs, loki, quickwit")
	...
	flag.StringVar(&cfg.Protocol, "protocol", "msgpack", "For iedb: msgpack or lineprotocol")
	...
	if cfg.Port == 0 {
		...
		if !ok {
			...
			fmt.Println("Valid targets: iedb, elastic, opensearch, clickhouse, clickhouse-native, victorialogs, loki, quickwit")
			os.Exit(1)
		}
		cfg.Port = port
	}
