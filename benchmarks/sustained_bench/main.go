// Sustained load benchmark for iedb ingestion
...
type Config struct {
	...
	Target      string // "iedb", "clickhouse", "clickhouse-http", "timescaledb", "influxdb3"
	InfluxPort  int
	Host        string
	Port        int
	CHPort      int
...
func main() {
	...
	flag.StringVar(&cfg.Target, "target", "iedb", "Target: iedb, clickhouse, clickhouse-http, timescaledb, influxdb3")
	flag.StringVar(&cfg.Host, "host", "localhost", "Server host")
	flag.IntVar(&cfg.Port, "port", 8000, "Server port (iedb HTTP)")
