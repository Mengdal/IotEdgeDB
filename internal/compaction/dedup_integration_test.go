...
func TestBuildCompactionQuery_DedupMixedTimeType(t *testing.T) {
	...
	// Output time must be TIMESTAMP WITH TIME ZONE (matches IEDB's ingest schema; UTC-anchored).
...
// of the dedup test above. A measurement whose Parquet files carry NO "iedb:tags"
...
// production/cpu/2026/06/18/03) against IEDB's real linked DuckDB.
