...
//
// Memory profiling: Set IEDB_COMPACTION_PROFILE=1 to enable heap profiling.
// Profiles are written to /tmp/iedb_compaction_heap_before.pprof and /tmp/iedb_compaction_heap_after.pprof
func RunSubprocessJob(config *SubprocessJobConfig) (*SubprocessJobResult, error) {
	...
	profilingEnabled := os.Getenv("IEDB_COMPACTION_PROFILE") == "1"
	if profilingEnabled {
	...
	if profilingEnabled {
		...
		writeHeapProfile(logger, "/tmp/iedb_compaction_heap.pprof")
	}
...
func RunJobInSubprocess(ctx context.Context, config *SubprocessJobConfig, logger zerolog.Logger, extraEnv ...string) (*SubprocessJobResult, error) {
	...
	// Build command: iedb compact --job-stdin
