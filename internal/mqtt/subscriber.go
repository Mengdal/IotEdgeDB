...
func (s *Subscriber) mapToRecord(m map[string]interface{}) (*models.Record, error) {
	...
	// Extract timestamp - iedb uses microseconds internally
	...
	// Special handling for "dn" (Device Name) -> treat as Tag
	if dn, ok := m["dn"].(string); ok {
		record.Tags["dn"] = dn
	}

	// Extract fields
	if fields, ok := m["fields"].(map[string]interface{}); ok {
		// Standard iedb format: "fields": {...}
		record.Fields = fields
	} else {
		// Custom format handling: flatten top-level keys
		for k, v := range m {
			// Skip reserved keys
			if k == "m" || k == "measurement" || k == "t" || k == "time" || k == "timestamp" || k == "tags" || k == "fields" || k == "dn" {
				continue
			}

			// Flatten "properties" or generic nested maps
			if nestedMap, ok := v.(map[string]interface{}); ok {
				for nk, nv := range nestedMap {
					// Use original nested key name if possible to keep it clean
					// e.g. properties.temperature -> temperature (if valid)
					// otherwise composite key
					record.Fields[nk] = nv
				}
			} else {
				record.Fields[k] = v
