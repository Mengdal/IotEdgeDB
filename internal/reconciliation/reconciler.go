// Package reconciliation implements Phase 5 of IEDB's cluster file-tracking
...
type Config struct {
	...
	// outside the standard IEDB layout (`db/m/yyyy/mm/dd/hh/file.parquet`,
	// 7 segments) are eligible for deletion. Default false — secure by
	// default for shared-bucket deployments where stray non-IEDB files
...
//     `<= 0` to skip the walk, and operators set `0` in iedb.toml to
