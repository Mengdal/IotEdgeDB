...
func (r *Reconciler) sweepOrphanStorage(
	...
	if r.cfg.BackendKind == BackendStandalone {
		// Standalone IEDB has no manifest — every storage file would be
		// classified as orphan-storage and (with DeletePreManifestOrphans
		// default true) deleted en masse. The wiring layer at
		// cmd/iedb/main.go gates reconciliation on clusterCoordinator
	...
	// doesn't have at least three segments (`<db>/<m>/<...>`). All IEDB
...
// looksLikeManagedPath returns true if a path conforms to the IEDB
...
// like IEDB data are touched, leaving stray files in mis-shared buckets
// alone.
//
