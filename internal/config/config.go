...
type LicenseConfig struct {
	...
	Key     string // License key (IEDB-ENT-XXXX-XXXX-XXXX-XXXX)
}
...
type ColdTierConfig struct {
	...
	S3AccessKey    string // AWS access key (use env: IEDB_TIERED_STORAGE_COLD_S3_ACCESS_KEY)
	S3SecretKey    string // AWS secret key (use env: IEDB_TIERED_STORAGE_COLD_S3_SECRET_KEY)
	S3UseSSL       bool   // Use HTTPS for S3 connections
	S3PathStyle    bool   // Use path-style addressing (required for MinIO)
	S3Prefix       string // Path prefix within the bucket (e.g., "instances/abc123/")
...
// ClusterConfig holds configuration for iedb clustering (Enterprise feature)
...
func setDefaults(v *viper.Viper) {
	...
	v.SetDefault("storage.local_path", "./data/iedb")
	...
	v.SetDefault("auth.db_path", "./data/iedb.db") // Shared SQLite DB (same as Python)
	...
	v.SetDefault("retention.db_path", "./data/iedb.db") // Shared SQLite DB with auth

	// Continuous query defaults
	v.SetDefault("continuous_query.enabled", true)             // Enable CQ management by default
	v.SetDefault("continuous_query.db_path", "./data/iedb.db") // Shared SQLite DB with auth
	...
	v.SetDefault("cluster.cluster_name", "iedb-cluster") // Default cluster name
...
func getDefaultMemoryLimit() string {
	...
	// Users can override via IEDB_DATABASE_MEMORY_LIMIT env var or config file
	cores := runtime.NumCPU()
