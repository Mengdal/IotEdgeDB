...
func ShouldForwardWrite(router *cluster.Router, c *fiber.Ctx) bool {
	...
	// Check for forwarding loop - if X-iedb-Forwarded-By is set, this request was already forwarded
...
const ShardRoutedHeader = "X-iedb-Shard-Routed"
...
func HandleShardRoutingError(c *fiber.Ctx, err error) error {
	...
	case errors.Is(err, sharding.ErrNoDatabaseHeader):
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": "Missing X-iedb-Database header required for sharded routing",
		})
