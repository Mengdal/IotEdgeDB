...
func validHeaders(t *testing.T, secret, peerNodeID, clusterName string) map[string]string {
	...
	return map[string]string{
		"X-IEDB-Node-ID":   peerNodeID,
		"X-IEDB-Cluster":   clusterName,
		"X-IEDB-Nonce":     nonce,
		"X-IEDB-Timestamp": strconv.FormatInt(ts, 10),
		"X-IEDB-HMAC":      mac,
	}
}
...
func TestCacheInvalidate_MissingHeaders(t *testing.T) {
	...
	cases := []struct {
		...
		{"missing X-IEDB-Node-ID", "X-IEDB-Node-ID"},
		{"missing X-IEDB-Cluster", "X-IEDB-Cluster"},
		{"missing X-IEDB-Nonce", "X-IEDB-Nonce"},
		{"missing X-IEDB-Timestamp", "X-IEDB-Timestamp"},
		{"missing X-IEDB-HMAC", "X-IEDB-HMAC"},
	}
	for _, c := range cases {
...
// claiming X-IEDB-Node-ID == this node's own ID is either misconfiguration
...
// — receiver checks the X-IEDB-Cluster header against its own
...
func TestCacheInvalidate_StaleTimestamp(t *testing.T) {
	...
	headers := map[string]string{
		...
		"X-IEDB-Cluster":   clusterName,
		"X-IEDB-Nonce":     nonce,
		"X-IEDB-Timestamp": strconv.FormatInt(staleTS, 10),
		"X-IEDB-HMAC":      mac,
	}
	if got := sendInvalidate(t, hx.app, headers); got != fiber.StatusForbidden {
...
// `X-IEDB-Timestamp: garbage` must 403, not 500. Defense against an
...
func TestCacheInvalidate_NonNumericTimestamp(t *testing.T) {
	...
	headers := validHeaders(t, secret, peerNodeID, clusterName)
	headers["X-IEDB-Timestamp"] = "not-a-number"
	if got := sendInvalidate(t, hx.app, headers); got != fiber.StatusForbidden {
...
func TestCacheInvalidate_NonceNotBurnedByBadMAC(t *testing.T) {
	...
	headers := validHeaders(t, secret, peerNodeID, clusterName)
	originalMAC := headers["X-IEDB-HMAC"]
	headers["X-IEDB-HMAC"] = "deadbeef" + originalMAC[8:] // wrong bytes, same length shape
	...
	headers["X-IEDB-HMAC"] = originalMAC
	if got := sendInvalidate(t, hx.app, headers); got != fiber.StatusNoContent {
