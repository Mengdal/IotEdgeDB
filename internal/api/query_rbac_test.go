...
// when x-iedb-database header is set, the RBAC check must resolve "default"
...
func TestEstimateQuery_RBAC_HeaderBypassFixed(t *testing.T) {
	...
	// With x-iedb-database: otherdb, the fixed checkQueryPermissions should remap
	// default → otherdb, causing the RBAC check to fail (otherdb not in allowedDBs).
	body := strings.NewReader(`{"sql": "SELECT * FROM cpu"}`)
	req := httptest.NewRequest("POST", "/api/v1/query/estimate", body)
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("x-iedb-database", "otherdb")
...
// when x-iedb-database header is set, cross-database syntax (db.table) must
...
// estimateQuery endpoint when x-iedb-database header is set.
...
// explicit `FROM db` checks permission against the x-iedb-database header (the
...
// x-iedb-database: secretdb the request must be denied (403), proving the check
// follows the header.
func TestExecuteQuery_ShowTables_HeaderDBScoped(t *testing.T) {
	...
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("x-iedb-database", "secretdb")
