...
func setupTestDatabasesHandler(t *testing.T, deleteEnabled bool) (*DatabasesHandler, *fiber.App, string) {
	...
	tmpDir, err := os.MkdirTemp("", "iedb-databases-test-*")
	if err != nil {
...
func TestDatabasesHandler_ListMeasurementsEmpty(t *testing.T) {
	...
	backend, _ := storage.NewLocalBackend(tmpDir, zerolog.Nop())
	defer backend.Close()
	backend.Write(ctx, "emptydb/.iedb-database", []byte(`{"created_at":"2025-01-01T00:00:00Z"}`))
...
func setupBenchmarkHandler(b *testing.B, numDatabases, measurementsPerDB int) (*DatabasesHandler, *fiber.App, *countingBackend, func()) {
	...
	tmpDir, err := os.MkdirTemp("", "iedb-databases-bench-*")
	if err != nil {
...
// exercised. Mirrors what cmd/iedb/main.go does at startup: mount the
...
func setupAuthedDatabasesHandler(t *testing.T) (*DatabasesHandler, *fiber.App, *auth.AuthManager, string) {
	...
	tmpDir, err := os.MkdirTemp("", "iedb-databases-authtest-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	...
	// collide with iedb-database marker files in the same dir.
	...
	app := fiber.New()
	// Mount the same global middleware cmd/iedb/main.go does. Without
...
// for iedb#471. Before the fix, POST /api/v1/databases passed through
