...
// sender (cmd/iedb/main.go) and the auth middleware's PublicRoutes
...
// cmd/iedb/main.go construct it conditionally and skip Register
// entirely otherwise.
//
// Closes audit finding #3 from 2026-05-19: the previous gate was a
// static header (`X-IEDB-Internal: cache-invalidate`) which any
...
// secret-string check inside the handler) means an unconfigured IEDB
...
func (h *CacheInvalidateHandler) handle(c *fiber.Ctx) error {
	nodeID := c.Get("X-IEDB-Node-ID")
	clusterName := c.Get("X-IEDB-Cluster")
	nonce := c.Get("X-IEDB-Nonce")
	mac := c.Get("X-IEDB-HMAC")
	tsStr := c.Get("X-IEDB-Timestamp")
