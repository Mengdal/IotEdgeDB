...
func (h *ImportHandler) handleFileImport(c *fiber.Ctx, format string, buildOpts func(*fiber.Ctx) importOptions) error {
	...
	if database == "" {
		...
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": "database is required (set x-iedb-database header or db query param)",
		})
	}
	...
	// Save to temp file
	tempDir, err := os.MkdirTemp("", "iedb-import-*")
	if err != nil {
		h.totalErrors.Add(1)
...
func (h *ImportHandler) importFile(ctx context.Context, dbName, measurement, filePath, tempDir string, opts importOptions) (*ImportResult, error) {
	...
	// Rename the time column to "time" if it has a different name, so iedb's
	...
	for _, hourStr := range partitionHours {
		// Parse the partition hour to construct iedb's storage path
		...
		// Generate iedb-standard storage path
...
// generateStoragePath creates an iedb-standard storage path for a partition
