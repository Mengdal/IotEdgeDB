...
// TestValidateHeaderDatabase covers the x-iedb-database header
...
func TestValidateSQLRequest_BypassesAndFalsePositives(t *testing.T) {
	tests := []struct {
		...
		// rejected to close the RBAC bypass vector. Only IEDB's transform
		...
		// iedb_partition_agg directly lets a caller scoped to db1 read
		// row counts in db2 because the function takes raw db/measurement
		// strings and globs the filesystem. Only IEDB's rewriter emits it.
		{name: "user iedb_partition_agg direct", sql: "SELECT * FROM iedb_partition_agg('db2', 'mem', 'hour')", shouldFail: true},
		{name: "user IEDB_PARTITION_AGG uppercase", sql: "SELECT * FROM IEDB_PARTITION_AGG('db2', 'mem', 'hour')", shouldFail: true},
		{name: "user iedb_partition_agg whitespace", sql: "SELECT * FROM iedb_partition_agg  ('db2', 'mem', 'hour')", shouldFail: true},
		{name: "user iedb_partition_agg in CTE", sql: "WITH x AS (SELECT * FROM iedb_partition_agg('db2', 'mem', 'hour')) SELECT * FROM x", shouldFail: true},
		{name: "literal containing iedb_partition_agg text", sql: "SELECT * FROM logs WHERE msg = 'iedb_partition_agg failed'", shouldFail: false},
		...
		// (e.g. a user scoped to db1 reads /data/iedb/db2/secrets via
		// parquet_scan). The strict table-function allowlist must reject
		// every one of them in user SQL.
		{name: "user parquet_scan alias", sql: "SELECT * FROM parquet_scan('/data/iedb/db2/secrets/**/*.parquet')", shouldFail: true},
		{name: "user PARQUET_SCAN uppercase", sql: "SELECT * FROM PARQUET_SCAN('/data/iedb/db2/*.parquet')", shouldFail: true},
		{name: "user parquet_scan whitespace", sql: "SELECT * FROM parquet_scan  ('/data/iedb/db2/*.parquet')", shouldFail: true},
		{name: "user parquet_scan in CTE", sql: "WITH x AS (SELECT * FROM parquet_scan('/data/iedb/db2/*.parquet')) SELECT * FROM x", shouldFail: true},
		{name: "user parquet_scan in JOIN", sql: "SELECT * FROM cpu JOIN parquet_scan('/data/iedb/db2/*.parquet') p ON cpu.id = p.id", shouldFail: true},
		{name: "user glob enumerate", sql: "SELECT * FROM glob('/data/iedb/**/*')", shouldFail: true},
		{name: "user read_blob raw bytes", sql: "SELECT * FROM read_blob('/data/iedb/db2/secrets/f.parquet')", shouldFail: true},
		{name: "user parquet_metadata stats", sql: "SELECT * FROM parquet_metadata('/data/iedb/db2/*.parquet')", shouldFail: true},
		{name: "user parquet_schema columns", sql: "SELECT * FROM parquet_schema('/data/iedb/db2/*.parquet')", shouldFail: true},
		...
		{name: "user read_json_auto", sql: "SELECT * FROM read_json_auto('/data/iedb/db2/x.json')", shouldFail: true},
		{name: "user read_json", sql: "SELECT * FROM read_json('/data/iedb/db2/x.json')", shouldFail: true},
		{name: "user read_text", sql: "SELECT * FROM read_text('/data/iedb/db2/x.txt')", shouldFail: true},
		{name: "user delta_scan", sql: "SELECT * FROM delta_scan('/data/iedb/db2/delta')", shouldFail: true},
		{name: "user iceberg_scan", sql: "SELECT * FROM iceberg_scan('/data/iedb/db2/ice')", shouldFail: true},
		...
		// review finding).
		{name: "parquet_scan via comma cross-join", sql: "SELECT * FROM cpu, parquet_scan('/data/iedb/db2/secrets/x.parquet')", shouldFail: true},
		{name: "read_parquet via comma cross-join", sql: "SELECT * FROM cpu, read_parquet('/data/iedb/db2/x.parquet')", shouldFail: true},
		{name: "glob via comma cross-join", sql: "SELECT * FROM cpu a, glob('/data/iedb/**/*') g", shouldFail: true},
		{name: "parquet_scan via nested subquery in FROM", sql: "SELECT * FROM (SELECT * FROM parquet_scan('/data/iedb/db2/x.parquet'))", shouldFail: true},
		{name: "glob via IN-list subquery", sql: "SELECT * FROM cpu WHERE host IN (SELECT file FROM glob('/data/iedb/**/*'))", shouldFail: true},
		{name: "read_blob via lateral join", sql: "SELECT * FROM cpu, LATERAL read_blob('/data/iedb/db2/x.parquet')", shouldFail: true},
		{name: "parquet_scan in scalar subquery", sql: "SELECT (SELECT count(*) FROM parquet_scan('/data/iedb/db2/x.parquet')) AS n", shouldFail: true},
		{name: "iedb_partition_agg via comma join", sql: "SELECT * FROM cpu, iedb_partition_agg('db2', 'mem', 'hour')", shouldFail: true},
		...
		{name: "double-quoted parquet_scan", sql: `SELECT * FROM "parquet_scan"('/data/iedb/db2/x.parquet')`, shouldFail: true},
		{name: "double-quoted read_parquet", sql: `SELECT * FROM "read_parquet"('/data/iedb/db2/x.parquet')`, shouldFail: true},
		{name: "double-quoted glob", sql: `SELECT * FROM "glob"('/data/iedb/**/*')`, shouldFail: true},
		{name: "backtick parquet_scan", sql: "SELECT * FROM `parquet_scan`('/data/iedb/db2/x.parquet')", shouldFail: true},
		{name: "double-quoted read_parquet via comma join", sql: `SELECT * FROM cpu, "read_parquet"('/data/iedb/db2/x.parquet')`, shouldFail: true},
...
// path-taking function (or IEDB loads an extension exposing one), add it both
...
func TestConvertSQLToStoragePathsWithHeaderDBUnicodeMeasurement(t *testing.T) {
	...
	result := h.convertSQLToStoragePathsWithHeaderDB(context.Background(), "SELECT * FROM 电表", "hb")
