...
// cluster<->local divergence shape., refusing the apply via
...
// with cluster truth until the operator re-syncs.
...
func (am *AuthManager) ApplyCreateToken(entry ClusterTokenEntry) error {
	...
	// timezone. Matches the RBAC apply path's UTC convention.
	//.
...
func (am *AuthManager) ApplyUpdateToken(entry ClusterTokenEntry) error {
	...
	if rows == 0 {
		...
		// snapshot and the row reappears.
		am.logger.Error().
...
// cache) until the cache TTL expires.
