...
// caller (cmd/iedb/main.go) typically gates this on
...
func (rm *RBACManager) SeedRBACFromLocalSQLite(ctx context.Context) error {
	...
	orgRows, err := func() ([]orgRow, error) {
		...
		// keeping the rows handle short-lived..
		// QueryContext so a cancelled ctx propagates to the driver and
		// frees the connection immediately..
	...
	// and flood the log with 4 useless Warns.
	// G24.
	if err := ctx.Err(); err != nil {
		if firstErr == nil {
			firstErr = err
	...
	// cancelled ctx releases the connection..
