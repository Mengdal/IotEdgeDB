...
func setupTestAuthManager(t *testing.T) (*AuthManager, func()) {
	...
	tmpDir, err := os.MkdirTemp("", "iedb-auth-test-*")
	if err != nil {
...
func TestNewAuthManager(t *testing.T) {
	...
	t.Run("cache config", func(t *testing.T) {
		tmpDir, _ := os.MkdirTemp("", "iedb-auth-test-*")
...
func TestForceAddRecoveryToken(t *testing.T) {
	...
	t.Run("recovery token has admin permissions", func(t *testing.T) {
		...
		if info.Name != "iedb-recovery" {
			t.Errorf("recovery token name = %q, want %q", info.Name, "iedb-recovery")
		}
	})
