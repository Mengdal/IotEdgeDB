...
type TokenInfo struct {
	...
	//Matches the sibling ExpiresAt pointer.
...
type AuthManager struct {
	...
	// entirely: the manager appeared to already have a shutdown signal,
...
func (am *AuthManager) EnsureInitialTokenWithValue(ctx context.Context, tokenValue string) (string, error) {
	...
	if result == "" {
		am.logger.Debug().Msg("IEDB_AUTH_BOOTSTRAP_TOKEN set but tokens already exist — skipping (no-op)")
	} else {
		am.logger.Info().Msg("First run detected - created initial admin token from IEDB_AUTH_BOOTSTRAP_TOKEN")
	}
	return result, nil
}
...
// happened to catch a cache miss in flight.
...
// file exists: "./data/iedb.db" and an absolute path to the same file are the
