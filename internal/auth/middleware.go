...
// Checks: x-iedb-database header, path parameter, query parameter
func extractDatabase(c *fiber.Ctx) string {
	// Check header first
	if db := c.Get("x-iedb-database"); db != "" {
		return db
	}
...
// Checks: x-iedb-measurement header, path parameter, query parameter
func extractMeasurement(c *fiber.Ctx) string {
	// Check header first
	if m := c.Get("x-iedb-measurement"); m != "" {
		return m
	}
