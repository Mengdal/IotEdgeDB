...
func (m *Migrator) MigrateFile(ctx context.Context, candidate MigrationCandidate) error {
	...
	// CRITICAL: Ensure we use normalized paths for metadata and S3 keys
	normalizedPath := filepath.ToSlash(candidate.Path)
	...
	if err := m.manager.metadata.UpdateTier(ctx, normalizedPath, candidate.TargetTier); err != nil {
		// Rollback: delete from destination
		if delErr := dstBackend.Delete(ctx, normalizedPath); delErr != nil {
			m.logger.Error().Err(delErr).Str("path", normalizedPath).Msg("Failed to rollback destination file")
		}
		return fmt.Errorf("failed to update tier metadata: %w", err)
	}

	// Delete from source tier
	// Note: Local backend handles both / and \ so normalized path is fine
	if err := srcBackend.Delete(ctx, normalizedPath); err != nil {
		m.logger.Warn().Err(err).Str("path", normalizedPath).Msg("Failed to delete source file after migration")
		...
		m.CleanupEmptyDirectories(ctx, normalizedPath)
	}
