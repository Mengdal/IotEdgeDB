...
type PolicyStore struct {
	...
	// re-cache an entry Delete had removed. Global rather than
...
func (s *PolicyStore) Get(ctx context.Context, database string) (*DatabasePolicy, error) {
	...
	// every lookup, twice per file per migration cycle. A cached nil
...
//     deleted policy until the next Set or restart. The possibly-
...
func (s *PolicyStore) List(ctx context.Context) ([]DatabasePolicy, error) {
	...
	// not-found entries, often the majority, and len(s.cache) would
	...
	for _, policy := range s.cache {
		// Skip cached negatives: nil marks "no custom policy" and
