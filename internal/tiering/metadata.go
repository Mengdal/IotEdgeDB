...
// DeleteFilesByDatabase removes all tier file records for a given database.
// Call this when dropping a database so stale measurements don't linger in
// the tiering metadata.
func (s *MetadataStore) DeleteFilesByDatabase(ctx context.Context, database string) (int64, error) {
	s.mu.Lock()
	defer s.mu.Unlock()

	// Collect measurements for cache invalidation before delete
	measQuery := `SELECT DISTINCT measurement FROM tier_files WHERE database = ?`
	rows, err := s.db.QueryContext(ctx, measQuery, database)
	if err != nil {
		return 0, fmt.Errorf("failed to query measurements: %w", err)
	}
	var measurements []string
	for rows.Next() {
		var m string
	...
	result, err := s.db.ExecContext(ctx, `DELETE FROM tier_files WHERE database = ?`, database)
	if err != nil {
		return 0, fmt.Errorf("failed to delete tier files: %w", err)
	}

	affected, _ := result.RowsAffected()

	// Invalidate tier cache for each affected measurement
	for _, m := range measurements {
		s.invalidateTierCache(database, m)
	}

	return affected, nil
}

// DeleteFilesByMeasurement removes all tier file records for a given database and measurement.
// Call this when deleting a measurement so stale entries don't linger in the tiering metadata.
func (s *MetadataStore) DeleteFilesByMeasurement(ctx context.Context, database, measurement string) (int64, error) {
	s.mu.Lock()
	defer s.mu.Unlock()

	result, err := s.db.ExecContext(ctx,
		`DELETE FROM tier_files WHERE database = ? AND measurement = ?`,
...
func (s *MetadataStore) CleanupOldMigrations(ctx context.Context, retentionDays int) (int64, error) {
	...
	// causing unnecessary write amplification. is
