...
func TestNoDuplication_NewPeriodicCycle(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		oldName := filepath.Join(tmpDir, "iedb-2024010"+string(rune('1'+i))+"_000000.wal")
...
func TestNoDuplication_MultipleNewCycles(t *testing.T) {
	...
	for cycle := 1; cycle <= 5; cycle++ {
		oldName := filepath.Join(tmpDir, "iedb-2024010"+string(rune('0'+cycle))+"_000000.wal")
...
func TestNoDuplication_FlushFailurePreservesWAL(t *testing.T) {
	...
	oldFiles := []string{
		filepath.Join(tmpDir, "iedb-20240101_000000.wal"),
		filepath.Join(tmpDir, "iedb-20240102_000000.wal"),
	}
...
func TestPurgeInactive_DeletesNonActiveFiles(t *testing.T) {
	...
	oldFiles := []string{
		filepath.Join(tmpDir, "iedb-20240101_100000.wal"),
		filepath.Join(tmpDir, "iedb-20240101_110000.wal"),
		filepath.Join(tmpDir, "iedb-20240101_120000.wal"),
	}
	for _, f := range oldFiles {
		header := make([]byte, WALFileHeaderSize)
		copy(header[0:4], WALMagic)
...
func TestPurgeInactive_ConcurrentWithWrite(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		name := filepath.Join(tmpDir, "iedb-2024010"+string(rune('1'+i))+"_000000.wal")
...
func TestPurgeAll_VsPurgeInactive(t *testing.T) {
	...
	activeFile := writer.CurrentFile()
	oldFile := filepath.Join(tmpDir, "iedb-20240101_000000.wal")
...
func TestRecovery_DeletesEmptyWALFile(t *testing.T) {
	...
	emptyFile := filepath.Join(tmpDir, "iedb-20240101_000000.wal")
	header := make([]byte, WALFileHeaderSize)
	copy(header[0:4], WALMagic)
	binary.BigEndian.PutUint16(header[4:6], WALVersion)
	header[6] = WALChecksumCRC32
...
func TestPurgeOlderThan_DeletesOldFiles(t *testing.T) {
	...
	recentFile := filepath.Join(tmpDir, "iedb-20240101_120000.wal")
...
func TestPurgeOlderThan_NoFilesToPurge(t *testing.T) {
	...
	os.WriteFile(filepath.Join(tmpDir, "iedb-20240101_000000.wal"), []byte("data"), 0600)
...
func TestPurgeAll_UsesSharedHelper(t *testing.T) {
	...
	// Create extra files
	os.WriteFile(filepath.Join(tmpDir, "iedb-20240101_000000.wal"), []byte("old"), 0600)
	os.WriteFile(filepath.Join(tmpDir, "iedb-20240102_000000.wal"), []byte("old"), 0600)
	writer.Close()
