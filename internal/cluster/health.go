...
//
//   - Zero compactors: operator forgot to set IEDB_CLUSTER_ROLE=compactor
...
//
// Both cases are non-fatal — IEDB keeps running, queries still work, but
	...
	if len(compactors) == 0 {
		...
		h.logger.Warn().
			Msg("No compactor elected: compacted files will accumulate. " +
				"Set IEDB_CLUSTER_ROLE=compactor on one node and restart, " +
