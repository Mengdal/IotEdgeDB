...
func TestShardRouterRouteWriteForward(t *testing.T) {
	...
	targetServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.NotEmpty(t, r.Header.Get("X-iedb-Forwarded-By"))
		assert.Equal(t, "true", r.Header.Get("X-iedb-Shard-Routed"))
	...
	req := httptest.NewRequest("POST", "/api/v1/write", strings.NewReader("data"))
	req.Header.Set("X-iedb-Database", "any_database")
