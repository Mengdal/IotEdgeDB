...
var (
	ErrNoDatabaseHeader   = errors.New("missing x-iedb-database header")
...
func (r *ShardRouter) RouteWrite(ctx context.Context, req *http.Request) (*http.Response, error) {
	...
	database := req.Header.Get("X-iedb-Database")
	if database == "" {
		database = req.Header.Get("x-iedb-database")
	}
	if database == "" {
...
func (r *ShardRouter) forward(ctx context.Context, node *cluster.Node, originalReq *http.Request) (*http.Response, error) {
	...
	req.Header.Set("X-iedb-Forwarded-By", r.localNode.ID)
	req.Header.Set("X-iedb-Shard-Routed", "true")
