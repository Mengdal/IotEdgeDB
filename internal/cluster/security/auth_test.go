...
// label), an in-progress rolling upgrade between IEDB versions will stop
...
// determinism test: a silent format change across an IEDB upgrade silently
...
// rolling-upgrade between IEDB versions stops being able to handshake
...
func TestDeriveReplicationSessionKey_Determinism(t *testing.T) {
	...
	const wantHex = "8bac5910d9a475830953e13531cfcacc1a9707f617fd037824a4aedcc58ee989"
	if gotHex != wantHex {
...
func TestComputeReplicationEntryTag_Determinism(t *testing.T) {
	...
	const wantHex = "3c6cc66321730df8"
	if gotHex != wantHex {
...
func TestDeriveReplicationSessionKey_KnownAnswer(t *testing.T) {
	const (
		...
		want = "98c6965ba2530fa345c270af8eed1b7caa1016eddfe9c46571904fc8ae822a69"
	)
