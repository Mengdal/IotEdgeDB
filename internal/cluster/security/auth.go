...
// domain separation. Defining a dedicated type — rather than using
...
// ValidateHMAC) for domain separation. Using shared typed constants —
...
// replayed against another within the freshness window — the same
...
// the cross-message-type replay this binding prevents.
...
//
// The info string "iedb-replication-session:" namespaces this KDF
...
func DeriveReplicationSessionKey(sharedSecret string, handshakeNonce string) ([]byte, error) {
	...
	key, err := hkdf.Key(sha256.New, []byte(sharedSecret), []byte(handshakeNonce), "iedb-replication-session:", 32)
	if err != nil {
...
// the hardcoded 5*time.Minute scattered across
// coordinator.go.
...
//
// At IEDB's 200k+ entries/sec ingest profile, reusing a per-reader
...
// At IEDB's 200k+ entries/sec ingest rate this saves both the hmac.New
// allocation per entry AND the sha256(payload) work the receiver
// already does for cumulativeHash..
