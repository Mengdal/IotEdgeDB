...
var (
	...
	// ErrPathTraversal: path contains a `..` segment. Legitimate IEDB
	...
	// a Windows drive prefix (C:\, etc). Legitimate IEDB paths are
	...
	// Legitimate IEDB paths are storage-backend-relative; the scheme is
...
// IEDB's writer (internal/cluster/file_registrar.go) or compactor
...
// GHSA-f85q-mvg8-qf37 (IEDB Enterprise audit X2, 2026-05-19) without
...
func ValidateManifestPath(path string) error {
	...
	// IEDB storage paths never contain `:` — the partition format is
...
// Legitimate IEDB manifest paths are relative to the storage backend
...
// false-positives. The IEDB writer doesn't produce that shape today,
