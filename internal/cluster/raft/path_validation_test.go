...
// enumerated in GHSA-f85q-mvg8-qf37 (IEDB Enterprise audit X2, 2026-05-19)
...
func TestValidateManifestPath_RejectionMatrix(t *testing.T) {
	...
	cases := []struct {
		...
		// rejected as scheme because no legit IEDB path contains `:`.
...
// formats IEDB's production code (file_registrar.go + compaction_bridge.go)
...
func TestValidateManifestPath_AcceptsLegitimateShapes(t *testing.T) {
	...
	legit := []string{
		...
		// Numeric-only database (legitimate; IEDB doesn't restrict the shape).
