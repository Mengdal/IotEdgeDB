...
type FileEntry struct {
	...
	Database      string    `json:"database"`       // IEDB database name
	Measurement   string    `json:"measurement"`    // IEDB measurement name
...
type ClusterFSM struct {
	...
	//     iedb_cluster_manifest_rejected_paths_total, exported via
...
//
//   - Proposer path (IEDB's Node.RegisterFile / .UpdateFile etc.):
//     hashicorp/raft delivers the error via future.Response(), and
//     IEDB's node.go propagates it to the caller. The leader's
...
// /api/v1/metrics as iedb_cluster_manifest_rejected_paths_total).
...
// rather than a confusing one (" write" vs "write")
// round-2 review.
...
func (f *ClusterFSM) applyUpdateToken(payload []byte, logIndex uint64) interface{} {
	...
	if changed["name"] {
		...
		// the tokensByName secondary index 		// the apply path is single-threaded and blocks the Raft commit
...
func (f *ClusterFSM) Restore(rc io.ReadCloser) error {
	...
	// symmetric to applyCreateTeam's UNIQUE enforcement.
	...
	// uniqueness checks in applyCreateToken/applyUpdateToken
	// review feedback).
...
func (f *ClusterFSM) GetFilesPaginated(cursor string, limit int) ([]*FileEntry, string, error) {
	...
	start := 0
	if cursor != "" {
		// sort.SehStrings returns the first index where keys[i] >= cursor.
