...
func (n *Node) Start() error {
	...
	if count := n.fsm.RejectedPathsCount(); count > 0 {
		...
		n.logger.Error().
			Int64("count", count).
			Msg("manifest path validation refused entries during snapshot restore — see per-entry Error logs tagged 'manifest path validation failed' for the offending paths. Sources: pre-validation IEDB version, OR prior compromise. Audit and verify they are not present in active queries.")
	}
	n.raft = ra
