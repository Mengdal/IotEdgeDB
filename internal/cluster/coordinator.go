...
import (
	...
	"errors"
	"fmt"
	"iedb/internal/cluster/filereplication"
...
// cache-invalidate fan-out wired in cmd/iedb/main.go) can reuse this
...
func generateNodeID() string {
	...
	if err != nil {
		...
		return fmt.Sprintf("iedb-%x", suffix)
	}
	// Use hostname + PID. In Kubernetes StatefulSets, the hostname is the
	// stable pod name (e.g. "iedb-writer-0"), and PID is always 1 inside a
	...
	// running multiple IEDB instances on the same host (e.g. dev/testing).
	//
...
func (c *Coordinator) startFilePullerLocked() error {
	...
	if c.cfg.SharedSecret == "" {
		return fmt.Errorf("peer replication requires IEDB_CLUSTER_SHARED_SECRET to be set")
	}

	// Build the fetch client. Reuses cluster TLS so peer transfers
...
//
// Known limitation: there is a sub-millisecond window
...
func (c *Coordinator) AcceptReplicationConnection(conn net.Conn, syncReq *replication.ReplicateSync) error {
	...
	//
	// This ordering closes two races caught:
