...
type RestoreOptions struct {
	...
	RestoreConfig   bool // restore iedb.toml (requires restart)
}
...
func (m *Manager) streamRestoreFile(ctx context.Context, srcPath, destPath string) (int64, error) {
	tmpFile, err := os.CreateTemp("", "iedb-restore-*.parquet")
	if err != nil {
...
func (m *Manager) restoreSQLite(ctx context.Context, backupID string) error {
	srcPath := fmt.Sprintf("%s/metadata/iedb.db", backupID)
...
// restoreConfig restores the iedb.toml config file from the backup.
...
func (m *Manager) restoreConfig(ctx context.Context, backupID string) error {
	srcPath := fmt.Sprintf("%s/config/iedb.toml", backupID)
	data, err := m.backupStorage.Read(ctx, srcPath)
	if err != nil {
