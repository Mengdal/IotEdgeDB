...
// This file owns IEDB's DuckDB sandbox: an `enable_external_access=false`
...
//
// Strategy:
//  1. Enumerate every directory prefix IEDB legitimately needs at query time
...
// and every SET GLOBAL IEDB needs to perform.
...
// buildAllowedDirectories enumerates every directory prefix IEDB legitimately
...
func buildAllowedDirectories(cfg *Config) []string {
	...
	// Primary Azure backend (when storage.backend = "azure"). IEDB's config
