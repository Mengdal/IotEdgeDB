...
echo ""
echo "=== Listing all files in iedb-test bucket ==="
mc ls -r local/iedb-test/production/cpu/ | head -50

echo ""
echo "=== Directory structure ==="
mc ls local/iedb-test/production/cpu/

echo ""
echo "=== Checking for day-level files ==="
echo "Looking for: production/cpu/2026/01/21/*.parquet"
DAY_FILES=$(mc ls local/iedb-test/production/cpu/2026/01/21/ 2>/dev/null | grep -E '\.parquet$' | grep -v '/')
if [ -z "$DAY_FILES" ]; then
...
HOURLY_DIRS=$(mc ls local/iedb-test/production/cpu/2026/01/21/ 2>/dev/null | grep '/$')
if [ -z "$HOURLY_DIRS" ]; then
    ...
    mc ls local/iedb-test/production/cpu/2026/01/21/$FIRST_HOUR/ | head -5
fi

echo ""
echo "=== Storage Backend ListDirectories() output ==="
echo "This is what iedb sees when it calls storage.ListDirectories():"
echo ""

# Simulate what iedb's ListDirectories would return
echo "For prefix 'production/cpu/2026/01/21/':"
mc ls local/iedb-test/production/cpu/2026/01/21/ | awk '{print $NF}'
