...
echo "1. Configure iedb with S3 storage"
...
echo "- iedb built from source and configured for S3/MinIO"
echo ""
...
echo ""
echo "iedb configuration (iedb.toml):"
echo "  [storage]"
echo "  backend = \"s3\""
echo "  s3_bucket = \"iedb-test\""
...
if ! pgrep -x "iedb" > /dev/null; then
    echo "ERROR: iedb is not running"
    echo "Please start iedb with S3 storage configured"
    exit 1
fi

# Get iedb endpoint
IEDB_URL="${IEDB_URL:-http://localhost:8000}"
echo "Using iedb at: $IEDB_URL"
echo ""

# Step 1: Create database
echo "Step 1: Creating database '$DB_NAME'..."
curl -s -X POST "$IEDB_URL/api/v1/databases" \
...
for i in {1..100}; do
    curl -s -X POST "$IEDB_URL/write?db=$DB_NAME" \
...
TOTAL_ROWS=$(curl -s "$IEDB_URL/api/v1/query?db=$DB_NAME" \
    -H "Content-Type: application/json" \
...
RESPONSE=$(curl -s "$IEDB_URL/api/v1/query?db=$DB_NAME" \
    -H "Content-Type: application/json" \
...
curl -s -X DELETE "$IEDB_URL/api/v1/databases/$DB_NAME?confirm=true" | jq .
echo ""
